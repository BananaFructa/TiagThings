        ...
        if (restrictedFluids.contains(fluid)) {
            ...
            if (reserverPos != null) {
                ...
                if (reserverEntity != null)
                    if (reserverEntity instanceof TileEntityFluidPumpAlternative && reserverEntity != this)
                        occupied = true;
            }
            ...
            if(world.getTotalWorldTime()%40 == (((getPos().getX()^getPos().getZ()))%40 + 40)%40) {
                if(closedList.isEmpty()) prepareAreaCheck();
                    ...
                    if(fs == null) closedList.remove(target);
                        ...
                        //if(Config.IEConfig.Machines.pump_placeCobble && placeCobble) world.setBlockState(pos, Blocks.COBBLESTONE.getDefaultState());
