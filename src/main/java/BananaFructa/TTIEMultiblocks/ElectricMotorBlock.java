...
public class ElectricMotorBlock extends Block {
    ...
    public TileEntity createTileEntity(World world, IBlockState state) {
        return new ElectricMotorTileEntity(state.getValue(IEProperties.FACING_HORIZONTAL));
    }
    ...
    public IBlockState getActualState(IBlockState state, IBlockAccess worldIn, BlockPos pos) {
        ElectricMotorTileEntity te = (ElectricMotorTileEntity) worldIn.getTileEntity(pos);
    ...
    public boolean onBlockActivated(World world, BlockPos pos, IBlockState state, EntityPlayer player, EnumHand hand, EnumFacing side, float hitX, float hitY, float hitZ) {
        if(!world.isRemote)
        {
            player.openGui(TTMain.INSTANCE, 5, world, pos.getX(), pos.getY(), pos.getZ());
            return true;
        }
