...
public class ContainerSiliconCrucible extends ContainerIEBase<TileEntitySiliconCrucible> {

    TileEntitySiliconCrucible tile;
    EntityPlayer player;
    public ContainerSiliconCrucible(InventoryPlayer inventoryPlayer, TileEntitySiliconCrucible tile) {
        ...
        this.addSlotToContainer(new Slot(tile.inventoryHandlers.get(0),0,43,23){
            @Override
            public boolean isItemValid(ItemStack stack) {
                return inventory.isItemValidForSlot(slotNumber,stack);
            }
        });
        this.addSlotToContainer(new Slot(tile.inventoryHandlers.get(0),1,43,45){
            @Override
            public boolean isItemValid(ItemStack stack) {
                return inventory.isItemValidForSlot(slotNumber,stack);
            }
        });
        this.addSlotToContainer(new Slot(tile.inventoryHandlers.get(1),0,118,34){
            @Override
            public boolean isItemValid(ItemStack stack) {
                return false;
            }
