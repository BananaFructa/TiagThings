...
public class ItemBlockTTBase extends ItemBlock
{
    private int[] burnTime;

    public ItemBlockTTBase(Block b)
    {
        super(b);
        if(((BlockTTBase)b).enumValues.length>1)
            setHasSubtypes(true);
        this.burnTime = new int[((BlockTTBase)b).enumValues!=null?((BlockTTBase)b).enumValues.length:1];
    }
    ...
    public String getUnlocalizedName(ItemStack stack) {
        return getUnlocalizedName() + "." + ((BlockTTBase)this.block).getStateFromMeta(stack.getItemDamage()).getValue(((BlockTTBase)this.block).property).toString().toLowerCase().replace("_child","");
    }
    ...
    {
        if(((BlockTTBase)block).hasFlavour(stack))
        {
            String subName = ((BlockTTBase)this.block).getStateFromMeta(stack.getItemDamage()).getValue(((BlockTTBase)this.block).property).toString().toLowerCase(Locale.US);
            String flavourKey = Lib.DESC_FLAVOUR+((BlockTTBase)this.block).name+"."+subName;
    ...
    public ItemBlockTTBase setBurnTime(int meta, int burnTime)
    {
    ...
    {
        if(!((BlockTTBase)this.block).canIEBlockBePlaced(world, pos, newState, side, hitX,hitY,hitZ, player, stack))
            return false;
        boolean ret = super.placeBlockAt(stack, player, world, pos, side, hitX, hitY, hitZ, newState);
        if(ret)
        {
            ((BlockTTBase)this.block).onIEBlockPlacedBy(world, pos, newState, side, hitX,hitY,hitZ, player, stack);
        }
        return ret;
    }
    ...
    {
        BlockTTBase blockIn = (BlockTTBase) this.block;
    ...
    public static class ItemBlockIENoInventory extends ItemBlockTTBase
    {
