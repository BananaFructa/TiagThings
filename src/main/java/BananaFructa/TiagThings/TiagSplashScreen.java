...
public class TiagSplashScreen
{
    ...
    private static int stageAnimation = 0;
    ...
    private static Texture[] tiagBackgroundTexture = new Texture[7];
    private static Texture[] tiagConveyor = new Texture[8];
    ...
    {
        //Utils.writeDeclaredField(SplashProgress.class,null,"mutex",mutex,true);
        ...
        backgroundColor =    0x004a7b;
        fontColor =          0xffffff;
        barBorderColor =     0xffffff;
        barColor =           0x004a7b;
        barBackgroundColor = getHex("barBackground", 0x004a7b);
        memoryGoodColor =    0x004a7b;
        memoryWarnColor =    0x004a7b;
        memoryLowColor =     0x004a7b;
        int white = 0xffffff;
        ...
        thread = new Thread(new Runnable()
        {
            ...
            public void run()
            {
                ...
                for (int i = 0;i < 7;i++) {
                    ResourceLocation r = new ResourceLocation(TTMain.modId,"gui/splash_" + (i+1) + ".png");
                    tiagBackgroundTexture[i] = new Texture(r, null);
                }
                for (int i = 0;i < 8;i++) {
                    ResourceLocation r = new ResourceLocation(TTMain.modId,"gui/loading_" + (i+1) + ".png");
                    tiagConveyor[i] = new Texture(r, null);
                }
                ...
                while(!done)
                {
                    ...
                    if (first != null) {
                        int s = Math.min(6,Math.max(0,first.getStep()-1));
                        setColor(white);
                        glPushMatrix();
                        glTranslatef(320 - (float) barWidth / 2 - 56, 20 - 191 + textHeight2, 0);

                        glEnable(GL_TEXTURE_2D);
                        tiagBackgroundTexture[s].bind();

                        glBegin(GL_QUADS);
                        tiagBackgroundTexture[s].texCoord(0, 0, 0);
                        glVertex2f(0, 0);
                        tiagBackgroundTexture[s].texCoord(0, 0, 1);
                        glVertex2f(0, 802);
                        tiagBackgroundTexture[s].texCoord(0, 1, 1);
                        glVertex2f(512, 802);
                        tiagBackgroundTexture[s].texCoord(0, 1, 0);
                        glVertex2f(512, 0);
                        glEnd();
                        glDisable(GL_TEXTURE_2D);
                    ...
                    tiagConveyor[stage].bind();
                    glTranslatef(320 + w/2 - 128, 240 + h/2 - 128, 0);
                    glTranslatef(0,0,0);

                    glBegin(GL_QUADS);
                    tiagConveyor[stage].texCoord(0, 0, 0);
                    glVertex2f(0, 0);
                    tiagConveyor[stage].texCoord(0, 0, 1);
                    glVertex2f(0, 128);
                    tiagConveyor[stage].texCoord(0, 1, 1);
                    glVertex2f(128, 128);
                    tiagConveyor[stage].texCoord(0, 1, 0);
                    glVertex2f(128, 0);
                    glEnd();
                    glDisable(GL_TEXTURE_2D);
            ...
            private void drawBar(ProgressManager.ProgressBar b,int w,int h)
            {
                glPushMatrix();
                // title - message
                setColor(fontColor);
                glTranslatef(0, barHeight+textHeight2+5, 0);
                glScalef(2, 2, 1);
                glEnable(GL_TEXTURE_2D);
                fontRenderer.drawString(b.getTitle() + " - " + b.getMessage(), 0, 0, white);
                glDisable(GL_TEXTURE_2D);
                glPopMatrix();
                // border
                glPushMatrix();
                ...
                int barProgressWidth = (barWidth - 2) * (b.getStep() + 1) / (b.getSteps() + 1);

                setColor(barColor);
                drawBox(barProgressWidth, barHeight - 2); // Step can sometimes be 0.
                // progress text
                ...
                glPushMatrix();
                GL11.glScissor(0,0,w/2 + barProgressWidth - barWidth/2, h);
                String progress = "" + b.getStep() + "/" + b.getSteps();
                glTranslatef(((float)barWidth - 2) / 2 - fontRenderer.getStringWidth(progress), 2, 0);
                setColor(fontColor);
                glScalef(2, 2, 1);
                glEnable(GL_TEXTURE_2D);
                fontRenderer.drawString(progress, 0, 0, white);
                glPopMatrix();

                glPushMatrix();

                GL11.glScissor(w/2 + barProgressWidth - barWidth/2,0,w-(w/2 + barProgressWidth - barWidth/2),h);

                glTranslatef(((float)barWidth - 2) / 2 - fontRenderer.getStringWidth(progress), 2, 0);
                setColor(fontColor);
                glScalef(2, 2, 1);
                glEnable(GL_TEXTURE_2D);
                fontRenderer.drawString(progress, 0, 0, backgroundColor);
                glPopMatrix();
            ...
            private void drawMemoryBar(int w,int h) {
                ...
                glScalef(2, 2, 1);
                glEnable(GL_TEXTURE_2D);
                fontRenderer.drawString("Memory Used / Total", 0, 0, white);
                glDisable(GL_TEXTURE_2D);
                glPopMatrix();
                // border
                glPushMatrix();
                ...
                GL11.glEnable(GL_SCISSOR_TEST);
                int barProgressWidth = (barWidth - 2) * (usedMemory) / (maxMemory);
                // progress text
    ...
    private static class Texture
    {
        ...
        public Texture(ResourceLocation location, @Nullable ResourceLocation fallback, boolean allowRP)
        {
            ...
            try
            {
                ...
                this.size = size;
                glEnable(GL_TEXTURE_2D);
                synchronized(TiagSplashScreen.class)
                {
                    name = glGenTextures();
